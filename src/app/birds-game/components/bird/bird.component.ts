...
import {BASE_BIRD_COLOR, BirdInfo, BirdState, Replaces} from 'src/app/shared/models/types';
...
export class BirdComponent extends ClickableOxDirective implements OnInit, OnDestroy {
  ...
  constructor(private elementRef: ElementRef,
    ...
    this.addSubscription(this.answerService.tryAnswer, z => {
      this.interactable = false;
    });
    this.addSubscription(this.birdsService.birdsInteractable, z => {
      this.interactable = z;
    });
  ...
  public birdOptionCorrectCheck(birdOption: BirdInfo, birdCorrectAnswer: BirdInfo,
    ...
    if (isAnswerBird) {
      ...
      if (!isCorrect) this.soundOxService.playSoundEffect('colorful-heights/sounds/wrongBird.mp3', ScreenTypeOx.Game)
    }
    if (isCorrect) {
